...
class MensagemController extends \BaseController {
	...
	public function postMsgGeral()
	{
		...
		foreach($users as $u)
		{
			...
			$m->remetente_id 	 = Auth::user()->id;
			$m->destinatario_id  = $u->id;
			$m->titulo 		 	 = 'Mensagem Geral';
			$m->mensagem 	 	 = Input::get('msg_geral');
			$m->visualizado  	 = 0;

			$m->save();
		}

		return Redirect::to('mensagens')->with('success', ['Mensagem em massa enviada.']);
	}
