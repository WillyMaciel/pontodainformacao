...
class MeusdadosController extends BaseController {
	...
	public function getIndex($id = null)
	{
		...
		if(Auth::user()->perfil != 1)
		{
			...
			if(!Auth::user()->pacote_id)
			{
				$tags = Auth::user()->company_tags;
				$tags_array = explode(',', $tags);

			}

			return View::make('meusdados.index', compact('user','centros','centro','ruas','id','pacotes','hoje','hojeDB','menu', 'tags_array'));
		}
		else
		{
	...
	public function postSave()
	{
		...
		if(isset($tags_free))
		{
			$user->company_tags = implode(',', $tags_free);
		}
